...
- For total_premium: This MUST be the all-in cost to the client EXCLUDING broker fees. Look for "Total Package Cost", "Total Policy Cost", "Total Premium with Tax", "Total Due", "Grand Total", "Total Amount Due", or any final total line. It includes base premium + surplus lines tax + stamping fee + fire marshal tax + inspection fees + FSLSO fees + EMPA surcharge + any other STATE-MANDATED taxes and fees. CRITICAL: Do NOT include broker fees (wholesale or retail) in total_premium. Broker fees are intermediary charges and must be excluded. If the quote shows both "Total Package Cost" (without broker fee) and "Total Cost of Policy" (with broker fee), ALWAYS use the LOWER number ("Total Package Cost") which excludes the broker fee. For example if GL premium is $8,874, surplus lines tax is $430.39, and stamping fee is $3.55, then total_premium = $8,874 + $430.39 + $3.55 = $9,307.94. Do NOT add any "Broker Fee" line item. CRITICAL: total_premium must ALWAYS be >= premium. If no single total line exists, calculate total_premium = premium + taxes_fees (excluding broker fees). PREFERRED: If the quote prints a "Total Package Cost" line, use that EXACT number as total_premium. DOUBLE-CHECK: After extracting premium and total_premium, verify that total_premium = premium + ALL individual tax/fee line items (excluding broker fee). If your calculated total doesn't match the document's stated total, use the document's stated "Total Package Cost" number.
