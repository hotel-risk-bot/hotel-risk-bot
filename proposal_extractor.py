...
def _apply_hotelbound_overrides(data: dict, hb: dict) -> None:
    ...
    # Override property.limits for HotelBound — the SOV table's Bldg Value / BPP Value / BI Value
    # are insured values (TIV components), NOT coverage limits. The only real limit on a HotelBound
    # Quotation Memorandum is the "Overall Program Limit of Liability any one Occurrence" from the
    # RISK DETAILS section. GPT frequently inflates BPP/Building/BI limits to the TIV amount and
    # produces a misleading limits breakdown. Replace with the single program limit.
    if hb_tiv > 0:
        _hb_limits = [
            {"description": "Overall Program Limit of Liability any one Occurrence",
             "limit": f"${hb_tiv:,.0f}"},
            {"description": "Business Income", "limit": "Actual Loss Sustained"},
        ]
        _old_limits = prop.get("limits") or []
        prop["limits"] = _hb_limits
        logger.info(
            f"HotelBound override: property.limits replaced ({len(_old_limits)} entries) -> "
            f"Overall Program Limit ${hb_tiv:,.0f}"
        )
...
- HOTELBOUND PROPERTY LIMITS RULE: HotelBound / RT Specialty Quotation Memorandum quotes use a SHARED PROGRAM LIMIT structure. The ONLY coverage limit is the "Overall Program Limit of Liability any one Occurrence" shown in the RISK DETAILS section (e.g., $20,075,000). The Building Value, BPP Value, and BI Value shown on the SOV table (Schedule of Values or "Cost Break Out By Location") are INSURED VALUES (TIV components), NOT coverage limits. Do NOT extract Bldg Value/BPP Value/BI Value as entries in the property.limits[] array. The property.limits[] array for HotelBound quotes should contain ONLY: (1) the Overall Program Limit, and (2) Business Income = "Actual Loss Sustained" (from the VALUATION section). Put the per-building TIV breakdown in coverage_by_location, not in the limits array.
- HOTELBOUND FORMS SEPARATION RULE: The "Quotation Memorandum" ATTACHMENTS section and the "important coverage limitations" bullet list (Late Notice, Roof/Roof Covering, Cosmetic Damage Exclusion, HVAC Equipment exclusion, Vacancy Clause, Property Cyber and Data Endorsement, Electronic Data Endorsement, Pre-Existing Damage Exclusion, Communicable Disease Endorsement, Marijuana Exclusion, Wind Driven Elements Limitation, War Risk and Terrorist Exclusion, Fungus/Wet Rot/Dry Rot, Asbestos Exclusion, Extortion and Ransomware, Digital Assets and Operations, etc.) are ALL PROPERTY forms — they belong ONLY in property.forms_endorsements. Do NOT copy these into umbrella.forms_endorsements, general_liability.forms_endorsements, or any other coverage section. The HotelBound HB-prefixed attachments (HB Evidence of Coverage, HB Shared Limits Disclosure, HB Claim Reporting Sheet, HB Manuscript, HB Terrorism Endorsement, HB Aggregate Layer Disclosure Endorsement, HB Scheduled Limits Per Location Subject to Margin Clause, HB Equipment Breakdown Endorsement, HB Schedule of Carriers with Terrorism) are ALL property-only — they describe the HotelBound property program structure and never apply to umbrella/excess liability. When a HotelBound property quote and a separate excess liability quote (e.g., Great Point Insurance / Paramount Real Estate Group / StarStone / Navigators) are uploaded together, extract umbrella.forms_endorsements ONLY from the excess liability quote's own forms/endorsements list (e.g., "Following Form Excess Liability Insurance Policy", "Abuse or Molestation Exclusion", "Anti Stacking of Limits Condition", "Communicable Diseases Exclusion", "Employment Related Practices Exclusion", "Fungi or Bacteria Exclusion", "Maritime Liability Exclusion", etc.).
